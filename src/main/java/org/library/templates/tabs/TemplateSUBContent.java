...
public class TemplateSUBContent extends TemplatesContent {
	...
	public TemplateSUBContent(String book, String sort, String site, int maxBookCount) {
	...
	public MustacheObject generateMustacheObject() {
		...
		TemplateSiteFunctions tsf = new TemplateSiteFunctions("sub", this.sort, this.site);
		...
		sub.setPrev_site("?tab=sub" + (sort.equals("") ? "" : "&amp;sort=" + this.sort) + (this.site.equals("1") ? "" : "&amp;site=" + (Integer.parseInt(this.site) - 1)));
		sub.setNext_site("?tab=sub" + (sort.equals("") ? "" : "&amp;sort=" + this.sort) + "&amp;site=" + (Integer.parseInt(this.site) + 1));
		...
		sub.setRight_arrow(result.size() >= this.maxBookCount);
